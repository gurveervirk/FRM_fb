...
router.get("/fetchallfac", fetchuser, async (req, res) => {
  try {
...
router.post(
  ...
  async (req, res) => {
    try {
      const { ins, branches, subs, empId } = req.body;
...
router.put("/updateFac/:id", fetchuser, async (req, res) => {
  const { branches, subs} = req.body;
  try {
...
router.delete("/deleteFac/:id", fetchuser, async (req, res) => {
  try {
